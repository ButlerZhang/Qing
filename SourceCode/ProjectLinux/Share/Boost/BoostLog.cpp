...
{
    const int ProgramID = 99999; //TODO

    boost::shared_ptr<boost::log::sinks::text_file_backend> Backend = boost::make_shared<boost::log::sinks::text_file_backend>(
        boost::log::keywords::file_name = m_LogDirectory + FileName + "_%Y%m%d_%H%M%S_" + std::to_string(ProgramID) + ".log",
...
void BoostLog::InitBaseSink(const std::string &LogFileName)
{
    ...
    BaseSink->set_filter(boost::log::expressions::attr<LogLevel>("Severity") >= LL_CORE);
...
void BoostLog::Initialize(const std::string &ProgramName, const std::string &WorkDirectory, bool IsOkToWrite)
{
    ...
    if (m_IsOkToWrite)
    {
        //const std::string &TempProgramName = ProgramName.empty() ? GetProgramName(false) : ProgramName;
        if (SetLogDirectory(WorkDirectory.empty() ? BoostFileSystem::GetWorkDirectory() : WorkDirectory, ProgramName))
        {
            InitBaseSink(ProgramName);
        }
    }
}
...
bool BoostLog::SetLogDirectory(const std::string &Directory, const std::string &ProgramName)
{
    if (BoostFileSystem::IsDirectory(Directory))
    {
        m_LogDirectory = Directory;
        if (m_LogDirectory[m_LogDirectory.size() - 1] != '/')
        {
            m_LogDirectory.append(BoostFormat("/log/%s/", ProgramName));
        }
