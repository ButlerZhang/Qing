...
HANDLE NetworkEnvironment::GetIOCP()
{
    if (g_hIOCompletionPort == NULL)
    {
        ...
        if (g_hIOCompletionPort == NULL)
        {
            g_Log.WriteError(BoostFormat("Create IO completion port error = %d.", WSAGetLastError()));
        }
        else
        {
            g_Log.WriteInfo("Created IO completion port succeed.");
        }
    }
...
bool NetworkEnvironment::StartupNetwork()
{
    ...
    if (InterlockedIncrement(&g_NetworkInitCounter) == 1)
    {
        ...
        g_Log.WriteError(BoostFormat("WSAStartup error = %d.", Result));
    }
...
bool NetworkEnvironment::ShutdownNetwork()
{
    ...
    if (InterlockedDecrement(&g_NetworkInitCounter) == 0)
    {
        ...
        g_Log.WriteError(BoostFormat("WSACleanup error = %d.", Result));
    }
