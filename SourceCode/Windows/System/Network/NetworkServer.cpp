...
bool NetworkServer::Start(const std::string &ServerIP, int Port)
{
    if (NetworkBase::Start(ServerIP, Port))
    {
        ...
        {
            g_Log.WriteInfo("Start succeed, NetworkServer ready.");
            return true;
        }
    }
...
bool NetworkServer::CreateAndStartListen()
{
    ...
    {
        g_Log.WriteError(BoostFormat("Create listen socket failed, error = %d.", WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo(BoostFormat("Create listen Socket = %I64d succeed.", m_ListenSocketContext->m_Socket));
    ...
    {
        g_Log.WriteError(BoostFormat("Bind listen socket to IOCP failed, error = %d.", WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo("Bind listen socket to IOCP succeed.");
    ...
    {
        g_Log.WriteError(BoostFormat("Listen socket bind failed, error = %d.", WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo(BoostFormat("Listen socket bind succeed, IP = %s, Port = %d.", GetLocalIP().c_str(), m_ServerPort));

    //开始进行监听
    //SOMAXCONN:Maximum queue length specifiable by listen
    if (listen(m_ListenSocketContext->m_Socket, SOMAXCONN) == SOCKET_ERROR)
    {
        g_Log.WriteError(BoostFormat("Listen error = %d.", WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo("Listening...");
    return true;
}
...
bool NetworkServer::InitializeAcceptExCallBack()
{
    ...
    if (SOCKET_ERROR == WSAIoctl(
        ...
        NULL))
    {
        g_Log.WriteError(BoostFormat("WSAIoctl can not get AcceptEx pointer, error = %d.", WSAGetLastError()));
        return false;
    }
    ...
    if (SOCKET_ERROR == WSAIoctl(
        ...
        NULL))
    {
        g_Log.WriteError(BoostFormat("WSAIoctl can not get GetAcceptExSockAddrs pointer, error = %d.", WSAGetLastError()));
        return false;
    }
...
bool NetworkServer::StartPostAcceptExIORequest()
{
    ...
    g_Log.WriteInfo(BoostFormat("Post %d AcceptEx request.", InitAcceptPostCount));
    return true;
}
...
void NetworkServer::GetClientAddress(SOCKADDR_IN &ClientAddress, IOCPContext &AcceptIOCPContext)
{
    ...
    LocalComputer MyComputer;
    g_Log.WriteInfo(BoostFormat("Connected, Socket(%I64d) = (Server[%s:%d], Client[%s:%d]), message = %s.",
...
bool NetworkServer::HandleError(const std::shared_ptr<IOCPSocketContext> &pSocketContext, DWORD ErrorCode)
{
    if (WAIT_TIMEOUT == ErrorCode)                          //超时
    {
        ...
        {
            g_Log.WriteInfo("QingNetwork time out, reconnecting....");
            return true;
        }
        else
        {
            g_Log.WriteInfo("Client disconnected.");
    ...
    {
        g_Log.WriteError(BoostFormat("IOCP error = %d, exit thread.", ErrorCode));
        return false;
    }
}
...
bool NetworkServer::PostAccept(IOCPContext &AcceptIOCPContext)
{
    ...
    {
        g_Log.WriteError(BoostFormat("Create new accept socket failed, error = %d.", WSAGetLastError()));
        return false;
    }
        ...
        &AcceptIOCPContext.m_Overlapped))       //本次重叠IO所要用到的重叠结构
    {
        ...
        {
            g_Log.WriteError(BoostFormat("Post accept failed, Socket = %I64d, IOCPContextID = %I64d, error = %d.",
                AcceptIOCPContext.m_AcceptSocket, AcceptIOCPContext.m_ContextID, WSAGetLastError()));
            return false;
        }
    }

    g_Log.WriteInfo(BoostFormat("Post accept succeed, Socket = %I64d, IOCPContextID = %I64d.",
...
bool NetworkServer::ProcessAccept(const std::shared_ptr<IOCPSocketContext> &pClientSocketContext, IOCPContext &AcceptIOCPContext)
{
    ...
    {
        g_Log.WriteError(BoostFormat("Process accept, associate with IOCP error = %d.", GetLastError()));
        return false;
    }
...
bool NetworkServer::ProcessRecv(const std::shared_ptr<IOCPSocketContext> &pClientSocketContext, IOCPContext &RecvIOCPContext)
{
    ...
    g_Log.WriteInfo(BoostFormat("Recv, Socket = %I64d, IOCPContextID =%I64d, Client = %s:%d, message = %s",
...
void NetworkServer::WorkerThread()
{
    ...
    while (WaitForSingleObject(m_hWorkerThreadExitEvent, 0) != WAIT_OBJECT_0)
    {
        ...
        {
            LocalComputer MyComputer;
            g_Log.WriteError(BoostFormat("Client %s:%d disconnected.",
