...
bool NetworkBase::Start(const std::string & ServerIP, int Port)
{
    ...
    {
        g_Log.WriteInfo("Start succeed, repeat start.");
        return false;
    }
    ...
    if (m_hWorkerThreadExitEvent == NULL)
    {
        g_Log.WriteError(BoostFormat("Create WorkerThreadExitEvent error = %d.", GetLastError()));
        return false;
    }
...
void NetworkBase::FillAddress(sockaddr_in &ServerAddress)
{
    ...
    if (m_ServerIP.empty())
    {
        ...
        g_Log.WriteInfo(BoostFormat("Fill address set IP = %s, Port = %d.", "INADDR_ANY", m_ServerPort));
    }
    else
    {
        inet_pton(AF_INET, m_ServerIP.c_str(), &(ServerAddress.sin_addr.s_addr));
        g_Log.WriteInfo(BoostFormat("Fill address set IP = %s, Port = %d.", m_ServerIP.c_str(), m_ServerPort));
    }
}
...
bool NetworkBase::CreateWorkerThread(int WorkerThreadCount)
{
    ...
    for (std::vector<WorkerThreadParam>::size_type ThreadIndex = 0; ThreadIndex < m_ThreadParamVector.size(); ThreadIndex++)
    {
        ...
        g_Log.WriteInfo(BoostFormat("Created worker thread, Index = %d, DEC_ID = %d, HEX_ID = %x.",
            m_ThreadParamVector[ThreadIndex].m_ThreadIndex,
            m_ThreadParamVector[ThreadIndex].m_ThreadID,
            m_ThreadParamVector[ThreadIndex].m_ThreadID));
    }

    g_Log.WriteInfo(BoostFormat("Created total = %d worker threads.", m_ThreadParamVector.size()));
    return true;
}
...
bool NetworkBase::PostRecv(IOCPContext &RecvIOCPContext)
{
    ...
    {
        g_Log.WriteError(BoostFormat("Post recv failed, Socket = %I64d, IOCPContextID = %I64d, error = %d.",
            RecvIOCPContext.m_AcceptSocket, RecvIOCPContext.m_ContextID, WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo(BoostFormat("Post recv succeed, Socket = %I64d, IOCPContextID = %I64d.",
...
bool NetworkBase::PostSend(IOCPContext &SendIOCPContext)
{
    ...
    if ((SOCKET_ERROR == nBytesRecv) && (WSA_IO_PENDING != WSAGetLastError()))
    {
        g_Log.WriteError(BoostFormat("Post send failed, Socket = %I64d, IOCPContextID = %I64d, error = %d.",
            SendIOCPContext.m_AcceptSocket, SendIOCPContext.m_ContextID, WSAGetLastError()));
        return false;
    }

    g_Log.WriteInfo(BoostFormat("Post send succeed, Socket = %I64d, IOCPContextID = %I64d.",
...
DWORD NetworkBase::CallBack_WorkerThread(LPVOID lpParam)
{
    ...
    g_Log.WriteInfo(BoostFormat("Worker Thread DEC_ID = %d, HEX_ID = %x start.", ThreadID, ThreadID));
    pNetwork->WorkerThread();
    g_Log.WriteInfo(BoostFormat("Worker Thread DEC_ID = %d, HEX_ID = %x exit.", ThreadID, ThreadID));
